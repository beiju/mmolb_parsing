...
pub fn parsed(ron_cache: &str) -> impl Iterator<Item = LoadedEvents> {
    std::fs::read_dir(ron_cache).unwrap().map(|entry| {
        ...
        let game_id = entry
            ...
            .unwrap()
            .strip_suffix(".ron")
            .expect("Should be passed .ron files")
            .to_string();
...
fn main() {
    ...
    let ron_cache = args
        .next()
        .expect("single argument \"ron_cache\" should be present");

    let mut result = Vec::new();
    for (i, mut events) in parsed(&ron_cache).enumerate() {
        println!(
            "Got events vec with {} events and {} bytes",
            events.events.len(),
            allocative::size_of_unique(&events.events)
        );
