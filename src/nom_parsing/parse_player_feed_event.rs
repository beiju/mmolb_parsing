...
use nom::{
    branch::alt,
    bytes::complete::tag,
    character::complete::{i16, u8},
    combinator::{cond, fail, opt},
    error::context,
...
impl<
        ...
        T: Parser<
            &'output str,
            Output = ParsedPlayerFeedEventText<&'output str>,
            Error = Error<'output>,
        >,
    > PlayerFeedEventParser<'output> for T
{
}
...
pub fn parse_player_feed_event(event: &FeedEvent) -> ParsedPlayerFeedEventText<&str> {
    let event_type = match &event.event_type {
        ...
        Err(e) => {
            ...
            return ParsedPlayerFeedEventText::ParseError {
                error,
                text: &event.text,
            };
        }
    ...
    match result.finish() {
        ...
        Ok((leftover, _)) => {
            tracing::error!(
                "{event_type} feed event parsed had leftover: {leftover} from {}",
                &event.text
            );
            let error = FeedEventParseError::FailedParsingText {
                event_type: *event_type,
                text: event.text.clone(),
            };
            ParsedPlayerFeedEventText::ParseError {
                error,
                text: &event.text,
            }
        }
    ...
    context(
        ...
        alt((
            feed_delivery("Delivery")
                .map(|delivery| ParsedPlayerFeedEventText::Delivery { delivery }),
            feed_delivery("Shipment")
                .map(|delivery| ParsedPlayerFeedEventText::Shipment { delivery }),
            feed_delivery("Special Delivery")
                .map(|delivery| ParsedPlayerFeedEventText::SpecialDelivery { delivery }),
            feed_event_door_prize.map(|prize| ParsedPlayerFeedEventText::DoorPrize { prize }),
            feed_event_equipped_door_prize
                .map(|prize| ParsedPlayerFeedEventText::DoorPrize { prize }),
...
fn attribute_gain<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        ...
        .map(
            |(player_name, amount, attribute)| ParsedPlayerFeedEventText::AttributeChanges {
                player_name,
                amount,
                attribute,
            },
...
fn attribute_equal<'output>(event: &'output FeedEvent) -> impl PlayerFeedEventParser<'output> {
    |input| {
        if event.after(Breakpoints::Season3) {
            (
                parse_terminated("'s "),
                try_from_word,
                delimited(tag(" was set to their "), try_from_word, tag(".")),
            )
                .map(|(player_name, changing_attribute, value_attribute)| {
            ...
            (
                ...
                try_from_word,
                delimited(
                    tag(" became equal to their current base "),
                    try_from_word,
                    tag("."),
                ),
            )
...
fn enchantment_s1a<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        ...
        .map(
            |(player_name, item, amount, attribute)| ParsedPlayerFeedEventText::Enchantment {
                player_name,
                item,
                amount,
                attribute,
...
fn enchantment_s1b<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        preceded(
            tag("The Item Enchantment was a success! "),
            parse_terminated("'s "),
        ),
        emojiless_item,
...
fn enchantment_s2<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        ...
        emojiless_item,
        preceded(
            (tag(" was enchanted with "), opt(tag("a ")), tag("+")),
            separated_pair(u8, tag(" "), try_from_word),
        ),
        delimited(
            tag(" and +"),
            separated_pair(u8, tag(" "), try_from_word),
        ...
        .map(|(player_name, item, (amount, attribute), enchant_two)| {
            ParsedPlayerFeedEventText::Enchantment {
                player_name,
                item,
                amount,
                attribute,
                enchant_two: Some(enchant_two),
                compensatory: false,
            }
        })
}
...
fn enchantment_compensatory<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        preceded(
            tag("The Compensatory Enchantment was a success! "),
            parse_terminated("'s "),
        ),
        emojiless_item,
        alt((
...
fn take_the_mound<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        ...
        .map(
            |(to_mound_player, to_lineup_player)| ParsedPlayerFeedEventText::TakeTheMound {
                to_mound_player,
                to_lineup_player,
            },
        )
...
fn take_the_plate<'output>() -> impl PlayerFeedEventParser<'output> {
    (
        ...
        .map(
            |(to_plate_player, from_lineup_player)| ParsedPlayerFeedEventText::TakeThePlate {
                to_plate_player,
                from_lineup_player,
            },
        )
...
fn modification<'output>() -> impl PlayerFeedEventParser<'output> {
    |input| {
        if let Ok((input, player_name)) = (parse_terminated(" lost the ")).parse(input) {
            ...
            let (input, modification) = parse_terminated(" Modification.")
                .map(ModificationType::new)
                .parse(input)?;
            Ok((
                input,
                ParsedPlayerFeedEventText::Modification {
                    player_name,
                    modification,
                    lost_modification: Some(lost_modification),
                },
            ))
        } else {
...
fn seasonal_durability_loss_happened(
    input: &str,
) -> IResult<'_, &str, ParsedPlayerFeedEventText<&str>> {
...
fn seasonal_durability_loss_blocked(
    input: &str,
) -> IResult<'_, &str, ParsedPlayerFeedEventText<&str>> {
    let (input, player_name) =
        parse_terminated("'s Prolific Greater Boon resisted Durability loss for Season ")
            .parse(input)?;
    let (input, season) = u32.parse(input)?;
    let (input, _) = tag(".").parse(input)?;
    ...
    context(
        ...
        alt((
            ...
            player_greater_augment_mod
                .map(|(player_name, modification, augment_name)| ParsedPlayerFeedEventText::GainedModificationFromGreaterAugment { player_name, modification, augment_name }),
        )),
    )
}

fn player_greater_augment_result(
    input: &str,
) -> IResult<'_, &str, ParsedPlayerFeedEventText<&str>> {
    let (input, (player_name, greater_augment)) = alt((
        parse_terminated(" gained +10 to all Defense Attributes.")
            .map(|p| (p, PlayerGreaterAugment::Plating)),
        terminated((parse_terminated(" gained +75 "), try_from_word), tag("."))
            .map(|(p, attribute)| (p, PlayerGreaterAugment::Headliners { attribute })),
        terminated((parse_terminated(" gained +50 "), try_from_word), tag("."))
            .map(|(p, attribute)| (p, PlayerGreaterAugment::StartSmall { attribute })),
    ))
    .parse(input)?;
...
fn player_retracted_greater_augment_result(
    input: &str,
) -> IResult<'_, &str, ParsedPlayerFeedEventText<&str>> {
    let (input, (player_name, greater_augment)) = alt((
        parse_terminated(" lost 0.1 from all Defense Attributes.")
            .map(|p| (p, PlayerGreaterAugment::Plating)),
        terminated(
            (parse_terminated(" lost 0.75 from "), try_from_word),
            tag("."),
        )
        .map(|(p, attribute)| (p, PlayerGreaterAugment::Headliners { attribute })),
        terminated(
            (parse_terminated(" lost 0.5 from "), try_from_word),
            tag("."),
        )
        .map(|(p, attribute)| (p, PlayerGreaterAugment::StartSmall { attribute })),
    ))
...
fn player_retroactive_greater_augment_result(
    input: &str,
) -> IResult<'_, &str, ParsedPlayerFeedEventText<&str>> {
    let (input, (player_name, greater_augment)) = alt((
        parse_terminated(" gained +0.1 to all Defense Attributes.")
            .map(|p| (p, PlayerGreaterAugment::Plating)),
        terminated(
            (parse_terminated(" gained +0.75 to "), try_from_word),
            tag("."),
        )
        .map(|(p, attribute)| (p, PlayerGreaterAugment::Headliners { attribute })),
        terminated(
            (parse_terminated(" gained +0.5 to "), try_from_word),
            tag("."),
        )
        .map(|(p, attribute)| (p, PlayerGreaterAugment::StartSmall { attribute })),
    ))
...
fn roster<'output>(_event: &'output FeedEvent) -> impl PlayerFeedEventParser<'output> {
    context(
        ...
        alt((
            player_relegated
                .map(|player_name| ParsedPlayerFeedEventText::PlayerRelegated { player_name }),
            player_moved.map(
                |(team_emoji, player_name)| ParsedPlayerFeedEventText::PlayerMoved {
                    team_emoji,
                    player_name,
                },
            ),
